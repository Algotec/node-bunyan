...
if (!runtimeEnv && typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope) {
    runtimeEnv = 'web_worker';
}
if (!runtimeEnv) {
 ...
 */
function getCaller3Info(optionsStackDepth) {
    if (this === undefined) {
    ...
    var stackDepth = optionsStackDepth || 2;
    Error.stackTraceLimit = stackDepth + 1;

    Error.prepareStackTrace = function (_, stack) {
        var caller = stack[stackDepth];
        if (sourceMapSupport) {
...
function Logger(options, _childOptions, _childSimple) {
    ...
    if (parent && _childSimple) {
        ...
        this.src = parent.src;
        this.stackDepth = parent.stackDepth;
    ...
    if (options.stream) {
        ...
        if (runtimeEnv === 'browser' || runtimeEnv === 'web_worker') {
            /*
...
function mkRecord(log, minLevel, args) {
    ...
    if (log.src && !rec.src) {
        rec.src = getCaller3Info(log.stackDepth);
    }
    rec.v = LOG_VERSION;
    ...
    return function () {
        ...
        if (!this._emit) {
            ...
            if (!_haveWarned[dedupKey]) {
                var caller = getCaller3Info(log.stackDepth);
        ...
        probes && probes[minLevel] && probes[minLevel].fire(function () {
                return [ str ||
